...
for i in range(len(T_new)):
    ER = [i/100. for i in range(0,200000)]
    ...
    for j in range(len(perc_new_alpha)):
        ...
        for k in range(len(ER)):
            min = opt.minimize(lambda x: obj_func(
                                                    f1( x, ER[k], P_new, alpha_=from_per_to_alpha( ER[k], perc_new_alpha[i] ), alpha_new_out = alpha_new_out_value) ,
...
for i in range(len(perc_new_alpha)):
    ...
    for j in range(len(ER)):
        H2_out_plot.append(100*f_0_ER_H2(ER[j],from_per_to_alpha(ER[j],perc_new_alpha[i])))
    H2_out_plot_final.append(H2_out_plot)
    plt.plot(H2_out_plot,min_val_final[i],label='initial_temperature:{}, $\epsilon$: {}, H2O: {}'.format(T_new[0],alpha_new_out_value,perc_new_alpha[i]))
    #plt.plot(ER,min_val_final[i],label='H2O:{}'.format(perc_new_alpha[i]))
    #plt.scatter(final_y[i], final_x[i],label='CO:{}'.format(perc_beta_input[i]), s = 2)
    plt.xlabel('H$_{2},$%')
    plt.ylabel('T$_{cross}$,K')
    #plt.xlabel('H$_{2}O$,%')
    #plt.ylabel('H$_{2}$,%')
f = open('H2O_cross_alpha_16.txt', 'w')
f.write(str(T_new[0]) + '\n')
for i in range(len(perc_new_alpha)):
    f.write(str(perc_new_alpha[i]) + '\n')
    for j in range(len(H2_out_plot_final[i])):
        f.write(str(H2_out_plot_final[i][j]) + ' ' + str(min_val_final[i][j]) + '\n')
f.close()
plt.legend()
plt.show()
