#defines critical point where 2 roots pass into 1 root (saturated vapor of water)
...
P_new = float(P_new)
T_new = [float(T_new)]
perc_new_alpha = hi_sat_pres(cs((T_new[0]-273.15)),P_new*101.325)
perc_new_alpha = [float(perc_new_alpha)]
k = int(100.-100*perc_new_alpha[0])
perc_new_beta = [i/100. for i in range(k)]
